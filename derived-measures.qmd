...
viewof dm_age_min = Inputs.range([0, 18], { label: "Min age (years)", step: 0.5, value: 0 })
viewof dm_age_max = Inputs.range([0, 18], { label: "Max age (years)", step: 0.5, value: 18 })
...
dm_filtered = {
  ...
  const lo = Math.min(dm_age_min, dm_age_max) * 12;
  const hi = Math.max(dm_age_min, dm_age_max) * 12;
  return dm_slice
    .filter((d) =>
