...
viewof pop_age_min = Inputs.range([0, 18], { label: "Min age (years)", step: 0.5, value: 0 })
viewof pop_age_max = Inputs.range([0, 18], { label: "Max age (years)", step: 0.5, value: 18 })
```
...
pop_rows = {
  ...
  const lo = Math.min(pop_age_min, pop_age_max) * 12;
  const hi = Math.max(pop_age_min, pop_age_max) * 12;
...
::: {.callout-note}
Counts come from the `transcript_by_speaker` table of childes-db 2021.1.
