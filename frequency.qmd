...
```{ojs setup-fq}
//| output: false
// DuckDB-WASM is imported only on this page, and only initialized on first
// use; the first query therefore takes ~10 s (wasm download + init)
...
// fetch and register a shard's bytes once; buffers are cached across queries
register_shard = async (s) => {
  const fname = `shard-${s}.parquet`;
  if (!registered.has(fname)) {
    const buf = new Uint8Array(
      await fetch(`slices/freq/${fname}`).then((r) => {
        if (!r.ok) throw new Error(`failed to fetch ${fname}`);
        return r.arrayBuffer();
      }));
    await (await db).registerFileBuffer(fname, buf);
...
// etl/write_freq_shards.py. hash_check.json carries build-time test vectors;
// any drift throws here, which surfaces as a visible cell error on the page
// (rather than silently querying the wrong shard and returning nothing).
...
::: {.callout-note}
Counts come from the childes-db 2021.1 type-frequency table (one row per
